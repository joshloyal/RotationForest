...
from sklearn.utils import resample, gen_batches
from sklearn.utils.extmath import fast_dot
from sklearn.decomposition import PCA

from _exceptions import DataConversionWarning, NotFittedError
...
class BaseRotationForest(six.with_metaclass(ABCMeta, BaseEnsemble,
                                    _LearntSelectorMixin)):
    """Base class for a rotation forests of trees.
    ...
    def __init__(self,
                 ...
                 warm_start=False,
                 class_weight=None):
        super(BaseRotationForest, self).__init__(
    ...
    def project(self, X, i):
        """ Apply the projection corresponding to the ith tree """
        return fast_dot(X, self.rotation_matrices_[i])
    ...
    def apply(self, X):
        ...
        results = Parallel(n_jobs=self.n_jobs, verbose=self.verbose,
                           backend="threading")(
            delayed(_parallel_helper)(tree, 'apply', self.project(X, i), check_input=False)
    ...
    def decision_path(self, X):
        ...
        indicators = Parallel(n_jobs=self.n_jobs, verbose=self.verbose,
                              backend="threading")(
            delayed(_parallel_helper)(tree, 'decision_path', self.project(X, i),
                                      check_input=False)
            for i, tree in enumerate(self.estimators_))
...
class ForestClassifier(six.with_metaclass(ABCMeta, BaseRotationForest,
                                          ClassifierMixin)):
    ...
    def predict_proba(self, X):
        ...
        all_proba = Parallel(n_jobs=n_jobs, verbose=self.verbose,
                             backend="threading")(
            delayed(_parallel_helper)(e, 'predict_proba', self.project(X, i),
                                      check_input=False)
...
class ForestRegressor(six.with_metaclass(ABCMeta, BaseRotationForest, RegressorMixin)):
    ...
    def predict(self, X):
        ...
        all_y_hat = Parallel(n_jobs=n_jobs, verbose=self.verbose,
                             backend="threading")(
            delayed(_parallel_helper)(e, 'predict', self.project(X, i), check_input=False)
            for i, e in enumerate(self.estimators_))
...
class RotationForestClassifier(ForestClassifier):
    ...
    n_subsets : integer, option (default=5)
        The number of rotation subsets used to build each tree.
